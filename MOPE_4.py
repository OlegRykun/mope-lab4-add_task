...
def lab4(n, m):
	...
	while True:
		...
		if cochrane(gp, gt):
			...
			if fisher(ft, fp):
				vur = "Рівняння регресії адекватне"
				print(vur)
				break
			else:
				vur = "Рівняння регресії не адекватне"
				print(vur)
				break
...
# цей цикл потрібний для повтору виклику функції, якщо рівнянн регресії не адекватно
# таких повторів в даній програмі десять, щоб не було зациклення
p = 0
while p <= 10:
	if vur == "Рівняння регресії не адекватне":
		lab4(n, m)
	p += 1